...
class RemoteKeyDataSourceImpl(
    ...
    override suspend fun getKey(id: String): RemoteKey? =
        withContext(Dispatchers.IO) { safeQuery(null) { queries.getRemoteKey(id).executeAsOneOrNull()?.toDomain() } }
    ...
    override suspend fun clearKeys() {
        withContext(Dispatchers.IO) { safeExecute { queries.clearRemoteKeys() } }
    }
}
